...
class YouTubeSearch:
    ...
    def _fetch_search_results(self, html, limit=10):
        ...
        search_end = ";</script>"
        videos_json_end = videos_json_start + html[videos_json_start:].find(search_end) - 1;
...
class ProviderYouTube(ProviderBase):
    ...
    def from_url(self, url, retries=5):
        ...
        try:
            logger.info("===================================================")
            logger.info("Searching métadata in '%s'" % url)
            content = pytube.YouTube(url)
        except urllib.error.HTTPError:
            logger.error("Erreur 404 non trouvé pour '%s'" % url)
            raise
        except KeyError:
