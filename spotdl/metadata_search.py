...
class MetadataSearch:
    ...
    def _best_on_youtube_search_for_type_query(self, query, spotify_md=None):
        ...
        if self.yt_manual:
            ...
            dur = spotify_md["duration_ms"] / 1000 if "duration_ms" in spotify_md else spotify_md["duration"]

            def duration_string_to_int(x, dur):
                x = x["duration"].split(":")
                acc = 0
            ...
            okval=100000000000000000000000000
            for v in videos:
                val=duration_string_to_int(v, dur)
                if val < okval:
                    ok=v
                    okval=val
    ...
    def _on_youtube_and_spotify_for_type_spotify(self, url):
        ...
        spotify_metadata = self._on_spotify_for_type_spotify(url)
        search_query = spotdl.metadata.format_string(self.yt_search_format, spotify_metadata)
        youtube_video = self._best_on_youtube_search_for_type_query(search_query, spotify_metadata)
